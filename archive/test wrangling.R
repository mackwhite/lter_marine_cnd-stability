...
# write_csv(dat_ready_2, "local_data/data_wrangling_test_new.csv")

dat_ready_2_old <- read_csv("local_data/model_data_all.csv")
...
vcr_test_old <- old |> 
      filter(project == "VCR") |> 
      group_by(site) |> 
      summarize(bm = mean(total_biomass),
                nsupply = mean(total_nitrogen),
                n_spp = mean(n_spp),
                s_struc = mean(max_size))
