...
### check for NAs
na_count_per_column <- sapply(dt_mutate_12, function(x) sum(is.na(x)))
...
dt_spp <- dt_mutate_12 |> 
      ...
      summarize(
            ...
            count = length(which(dmperind_g_ind != 0)),
            spp_max_size = mean(max_size, na.rm = TRUE),
            spp_var_size = mean(var_size, na.rm = TRUE)) |> 
      ungroup() |>
...
dt_spp_clean <- dt_spp |>
      select(-spp_max_size, -spp_var_size)
glimpse(dt_spp_clean)
...
dt_spp_strata <- left_join(dt_spp_clean, 
                             strata_list1, 
...
write_csv(model_dt_1, "local_data/species-level-nutrient-stability_10172024.csv")