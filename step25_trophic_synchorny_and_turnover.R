...
librarian::shelf(tidyverse, googledrive, vegan, readxl, codyn, purrr, splitstackshape)
...
dt_mutate_0_pisco_1 <- dt_mutate_0_pisco |> filter(count > 0) |> splitstackshape::expandRows(count = "count", drop = FALSE) |> 
...
### filter where count is greater than zero [keep for now so you can check] and duplicate by count to make each row an individual
dt_mutate_0_sbc_1 <- dt_mutate_0_sbc |> filter(count > 0) |> splitstackshape::expandRows(count = "count", drop = FALSE) |> 
      ### account for the step above by dividing density by count so we aren't artificially inflating the density and thus nsupply below
      mutate(density_num_m2 = density_num_m2/count)
### bind zero-data with longer, individual-per-row data
...
species_presence <- dat_ready_4 |> 
      ...
      group_by(program, year, month, site, subsite_level1, subsite_level2, subsite_level3, diet_cat) |> #remove habitat, because different than what it came in as - dont need it here
